...
QString vtfFormatToString(ImageFormat format) {
	...
	static const QMap<ImageFormat, QString> formatToString = {
		...
		{ TITANFALL_BC6H, "TITANFALL_BC6H" },
		{ TITANFALL_BC7, "TITANFALL_BC7" },
		{ STRATA_R8, "STRATA_R8" },
		{ STRATA_BC7, "STRATA_BC7" },
		{ STRATA_BC6H, "STRATA_BC6H" },
		{ SOURCEPP_BGRA8888_HDR, "BGRA8888_HDR" },
		{ SOURCEPP_RGBA16161616_HDR, "RGBA16161616_HDR" },
		{ SOURCEPP_CONSOLE_RGBA16161616_HDR, "CONSOLE_RGBA16161616_HDR" },
	};
...
void VTFWidget::decodeImage(int mip, int frame, int face, int slice, bool alpha) {
	...
	this->image = QImage(reinterpret_cast<uchar*>(this->imageData.data()), this->vtf->getWidth(this->currentMip), this->vtf->getHeight(this->currentMip), QImage::Format_RGBA8888);
	if (!this->alphaEnabled) {
