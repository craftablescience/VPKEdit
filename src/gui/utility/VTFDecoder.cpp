...
std::optional<VTFData> VTFDecoder::decodeImage(const CVTFFile& vtf, bool alpha) {
	return decodeImage(vtf, 0, 0, 0, alpha);
}
...
std::optional<VTFData> VTFDecoder::decodeImage(const CVTFFile& vtf, int face, int frame, int mip, bool alpha) {
	...
	std::unique_ptr<std::byte[]> imageData{new std::byte[size]};
	bool ok = CVTFFile::Convert(vtf.GetData(frame, face, 0, mip), reinterpret_cast<vlByte*>(imageData.get()), imageWidth, imageHeight, vtf.GetFormat(), format);
	if (!ok) {
		return std::nullopt;
	}
	return VTFData{
