...
struct EntryContextMenuData {
	explicit EntryContextMenuData(bool useRoot, QWidget* parent = nullptr) {
		...
		this->contextMenuVirtualFile = new QMenu(parent);
		this->extractVirtualFileAction = this->contextMenuVirtualFile->addAction(parent->style()->standardIcon(QStyle::SP_DialogSaveButton), QObject::tr("Save As..."));
		this->contextMenuVirtualFile->addSeparator();
		this->overwriteVirtualFileAction = this->contextMenuVirtualFile->addAction(parent->style()->standardIcon(QStyle::SP_DialogResetButton), QObject::tr("Overwrite..."));
	}
	...
	void setReadOnly(bool readOnly) const {
		...
		this->overwriteVirtualFileAction->setDisabled(readOnly);
	}
	...
	QMenu* contextMenuVirtualFile = nullptr;
	QAction* extractVirtualFileAction = nullptr;
	QAction* overwriteVirtualFileAction = nullptr;
};
